...
def setup():
    ...
    if request.method == 'GET' and 'zip_link' in request.args and 'proj_name' in request.args:
        session['proj_name'] = request.args.get('proj_name')
        zip_link = request.args.get('zip_link')
        try:
            # Download and extract zip
            zip_file = f"/tmp/{session['proj_name']}.zip"
            subprocess.run(["wget", "-O", zip_file, zip_link], check=True)
            extract_path = f"/tmp/{session['proj_name']}_extracted"
            os.makedirs(extract_path, exist_ok=True)
            shutil.unpack_archive(zip_file, extract_path)

            # Set ONNX folder to extracted path
            session['onnx_folder'] = extract_path
        except Exception as e:
            return jsonify({"error": f"Failed to process zip file: {str(e)}"}), 500

        session['netron_start_port'] = 20123
        session['setup_complete'] = True
        return redirect(url_for('index'))
...
def run():
    app.run(debug=False, port=30129)
